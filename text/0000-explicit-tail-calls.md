...
If this guarantee cannot be provided by the compiler, a compile-time error is generated instead.
...
The other goal is to avoid paying the cost to create a new stack frame, replacing `call` instructions with `jmp` instructions; this optimization has performance implications and can provide massive speedups for algorithms that have a high density of function calls. This goal also depends on the guarantee as otherwise a subtle change or a new compiler version can have an unexpected impact on performance.
...
For the second goal, TCO can have the intended effect; however, there is no guarantee. This can result in unexpected slow-downs as can be seen in this [issue](https://github.com/rust-lang/rust/issues/102952).
...
overflowing the stack, and guaranteeing significantly faster interpreters and emulators. One common example of the
...
With this feature, this approach would also be possible in Rust.
...
In this example, the function would need to be evaluated and **then** the addition would need to take place.
...
The stack frame of the calling function is reused, so it is essentially cleaned up and the called function takes the space.
As a result, it is not possible to pass references to local variables, nor will the called function "return" to the calling function. All variables not used as an argument are dropped before the call and no cleanup will be done after the call.
...
or to have access to their variable values while debugging.
...
understanding of some advanced concepts and the current use cases are likely to be niche, so it should be taught similarly
...
instead of after. So in the following function ([original example](https://github.com/rust-lang/rfcs/issues/2691#issuecomment-1136728427)):
```rust
fn x() {
    let a = Box::new(());
    let b = Box::new(());
...
Here one function uses `become` and the other `return`; this is another potential source of confusion. This mutual recursion
...
- The argument to `become` is a function (or method) call that exactly matches the function signature and calling convention of the callee. The intent is to ensure a compatible stack frame layout.
- The stack frame of the calling function is reused, implying that the function is never returned to. The required checks to ensure this is possible are as follows: no borrows of local variables are passed to the called function (passing local variables by copy/move is ok, since that doesn't require the local variable to continue existing after the call), and no further cleanup is necessary. These checks can be done by using the borrowchecker as described in the [Borrowchecking](#borrowchecking) section below.

If any of these checks fail, a compile-time error is issued.

One additional check must be done: if the backend cannot guarantee TCE to be performed, a compile-time error is issued. It is also suggested to ensure that the invariants provided by the pre-requisites are maintained during compilation, issuing an error if this is not the case.
...
This feature will have interactions with other features that depend on stack frames such as debugging and backtraces. See [drawbacks](#drawbacks) for further discussion.
...
Invocations of operators were considered as valid targets, but they were rejected on the grounds of being too error-prone.
In any case, they can still be called as methods. One example of their error-prone nature ([source](https://github.com/rust-lang/rfcs/pull/3407#discussion_r1167112296)):
```rust
...
Further confusion could result from the same-signature restriction where the Rust compiler complains that `fibonacci` and `<u64 as Add>::add` do not share a common signature.
...
There are some designs that either cannot achieve the same performance or functionality as the chosen approach. Though most other designs revolve around how to mark what should be a tail call or marking what functions can be tail called, there is also the possibility of providing support for a custom backend (e.g. LLVM) or MIR pass.
...
of using constant stack space, though they cannot be used to achieve the performance that the chosen design is capable
of.
...
function definition. As sometimes a call should be guaranteed to be TCE and sometimes not, marking a function would
be less flexible.
...
This alternative mostly comes down to taste (or bikeshedding) and `become` was chosen as it is [reserved](https://rust-lang.github.io/rfcs/0601-replace-be-with-become.html) for this use, is shorter to write, and as drop order changes compared to `return`, a new keyword seems warranted.
...
## GCC
GCC does not support a feature equivalent to Clang's `musttail`, and there does not seem to be push to implement it ([pipermail](https://gcc.gnu.org/pipermail/gcc/2021-April/235882.html)) (as of 2021). However, there also exists an experimental [plugin](https://github.com/pietro/gcc-musttail-plugin) for GCC, last updated in 2021.


## Zig
Zig provides separate syntax to allow more flexibility than normal function calls. There are options for async calls, inlining, compile time evaluation of the called function, and specifying TCE on the call.
...
## .Net
The .Net JIT does support TCE as of 2020; a main motivator for this feature was improving performance ([pull request](https://github.com/dotnet/runtime/pull/341) and [issue](https://github.com/dotnet/runtime/issues/2191)).
...
- What parts of the design do you expect to resolve through the RFC process before this gets merged?
    - One point that needs to be decided is if TCE should be a feature that needs to be required from all backends, or if it can be optional. Currently, the RFC specifies that a compile-time error should be issued if a backend cannot guarantee that TCE will be performed.
...
- What parts of the design do you expect to resolve through the implementation of this feature before stabilization?
    - Are all calling conventions used by Rust available for TCE with the proposed restrictions on function signatures?
    ...
    - Can functions outside the current crate be supported, as well as functions from dynamically loaded libraries?
